...
/**
 * Ambil unique ID untuk sebuah stream, support IDN & Showroom.
 *
 * - IDN       → punya chat_room_id  → pakai "idn-{chat_room_id}"
 * - Showroom  → punya room_id       → pakai "sr-{room_id}"
 * - Fallback  → pakai url_key       → pakai "uk-{url_key}"
 */
function getStreamCacheId(stream) {
  const type = (stream.type || "").toLowerCase();
  if (type === "idn" && stream.chat_room_id) {
    return "idn-" + String(stream.chat_room_id);
  }
  if (type === "showroom" && stream.room_id) {
    return "sr-" + String(stream.room_id);
  }
  // Fallback universal
  return "uk-" + String(stream.url_key || stream.slug || Math.random());
}
